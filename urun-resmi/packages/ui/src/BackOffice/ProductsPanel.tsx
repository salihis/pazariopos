...
export function ProductsPanel({ initialCreateValues, onProductCreated }: ProductsPanelProps = {}) {
  ...
  const handleImageSelected = useCallback(async (file: File | undefined) => {
    ...
    try {
      const updated = await productsApi.uploadProductImage(editingProduct.id, file)
      setEditingProduct(updated)
      await load()
    } catch (err) {
      setImageUploadError(err instanceof Error ? err.message : 'Resim yüklenemedi.')
    } finally {
  ...
  const handleRemoveImage = useCallback(async () => {
    ...
    setImageUploadError(null)
    try {
      const updated = await productsApi.deleteProductImage(editingProduct.id)
      setEditingProduct(updated)
      await load()
    } catch (err) {
      setImageUploadError(err instanceof Error ? err.message : 'Resim silinemedi.')
    } finally {
      setIsUploadingImage(false)
    }
  }, [editingProduct, load])
    ...
    <div className="space-y-4">
      ...
      {showForm && (
        <div className="rounded-2xl border border-[var(--color-paper-line)] bg-[var(--color-paper-dim)] p-5">
          ...
          {/* Ürün Resmi — sadece kaydedilmiş bir ürün üzerinde yüklenebilir
              (yükleme endpoint'i bir ürün id'si gerektiriyor). Yeni ürün
              oluştururken önce kaydedip sonra düzenleme ekranından resim
              ekleyebilirsiniz. */}
          <div className="mb-3">
            <label className={labelClass}>Ürün Resmi</label>
            {!editingProduct ? (
              <p className="text-xs text-[var(--color-ink-soft)]">
                Resim eklemek için önce ürünü kaydedin, ardından düzenleme ekranından yükleyin.
              </p>
            ) : (
              <div className="flex items-center gap-3">
                <div className="flex h-20 w-20 shrink-0 items-center justify-center overflow-hidden rounded-lg border border-[var(--color-paper-line)] bg-white">
                  {editingProduct.imageUrl ? (
                    <img
                      src={`${getApiBaseUrl()}${editingProduct.imageUrl}`}
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  {editingProduct.imageUrl && (
                    <button
                      ...
                      disabled={isUploadingImage}
                      className="self-start text-xs font-medium text-[var(--color-copper)] hover:underline disabled:opacity-50"
                    >
                      Resmi Kaldır
                    </button>
                  )}
                  {isUploadingImage && <span className="text-xs text-[var(--color-ink-soft)]">Yükleniyor…</span>}
                  {imageUploadError && <span className="text-xs text-[var(--color-copper)]">{imageUploadError}</span>}
                </div>
              </div>
            )}
          </div>
          ...
          <div className="overflow-x-auto">
            ...
            <thead>
              <tr className="receipt-rule text-xs text-[var(--color-ink-soft)]">
                <th className="pb-2 pt-1 text-left font-medium"></th>
            ...
            <tbody>
              {filtered.map(p => (
                <tr key={p.id} className={`border-b border-[var(--color-paper-line)]/60 ${!p.isActive ? 'opacity-50' : ''}`}>
                  <td className="py-1.5">
                    <div className="flex h-9 w-9 items-center justify-center overflow-hidden rounded-md border border-[var(--color-paper-line)] bg-white">
                      {p.imageUrl ? (
                        <img src={`${getApiBaseUrl()}${p.imageUrl}`} alt={p.name} className="h-full w-full object-cover" />
                      ) : (
