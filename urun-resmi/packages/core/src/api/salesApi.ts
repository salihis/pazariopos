...
/** Products' imageUrl is server-relative (e.g. "/api/uploads/products/x.jpg") —
 * callers that need to render it as an <img src> prepend this. */
...
export const productsApi = {
  ...
  // Deliberately bypasses `request()` — it hardcodes Content-Type:
  // application/json, which would stop the browser from setting the
  // multipart/form-data boundary header a FormData body needs.
  async uploadProductImage(id: string, file: File | Blob): Promise<Product> {
    const formData = new FormData()
    formData.append('file', file)
    const res = await fetch(`${apiBase}/api/products/${id}/image`, {
      method: 'POST',
  ...
  deleteProductImage(id: string): Promise<Product> {
    return request<Product>(`/api/products/${id}/image`, { method: 'DELETE' })
  },
}
